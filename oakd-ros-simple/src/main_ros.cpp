...
int main(int argc, char **argv)
{
  ...
  if (oak_handler.get_rgb || oak_handler.get_gray){  // RGB 또는 Gray 이미지가 필요한 경우
    ...
    rgb_thread = std::thread([&]() {
      ...
      while(ros::ok()){
        ...
        if (inPassRgb != nullptr){
          ...
          // Grayscale 이미지 변환 및 발행
          if (oak_handler.get_gray) {
            cv::Mat gray_frame;
            cv::cvtColor(FrameRgb, gray_frame, cv::COLOR_BGR2GRAY);
            cv_bridge::CvImage bridge_gray = cv_bridge::CvImage(header, sensor_msgs::image_encodings::MONO8, gray_frame);
            
            if (oak_handler.get_raw) {
              bridge_gray.toImageMsg(oak_handler.gray_img_msg);
              oak_handler.gray_pub.publish(oak_handler.gray_img_msg);
            }
            if (oak_handler.get_compressed) {
              bridge_gray.toCompressedImageMsg(oak_handler.gray_img_comp_msg);
              oak_handler.gray_comp_pub.publish(oak_handler.gray_img_comp_msg);
            }
          }
        }
