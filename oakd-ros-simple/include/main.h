...
class oakd_ros_class{
  public:
    ...
    int fps_IMU, infer_img_width, infer_img_height, class_num, thread_num, depth_confidence;
    ...
    // Add new message and publisher for gray image
    sensor_msgs::Image gray_img_msg;
    sensor_msgs::CompressedImage gray_img_comp_msg;
    ros::Publisher gray_pub, gray_comp_pub;
    bool get_gray;
    ...
    oakd_ros_class(ros::NodeHandle& n) : nh(n){
      ...
      // Add new parameter
      nh.param<bool>("/get_gray", get_gray, false);
      ...
      if (get_gray) {
        if (get_raw)
          gray_pub = nh.advertise<sensor_msgs::Image>(topic_prefix+"/gray/image_raw", 10);
        if (get_compressed)
          gray_comp_pub = nh.advertise<sensor_msgs::CompressedImage>(topic_prefix+"/gray/image_raw/compressed", 10);
      }
...
void oakd_ros_class::main_initialize(){
  ...
  if(get_rgb || get_gray) {  // RGB 또는 Gray 이미지가 필요한 경우
